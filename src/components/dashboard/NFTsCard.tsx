...
export function NFTsCard({ nfts, loading, network = "mainnet" }: NFTsCardProps) {
  ...
  const paginatedNFTs = React.useMemo(() => {
    ...
    return filteredAndSortedNFTs.slice(startIndex, startIndex + itemsPerPage);
          ...
          <>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center text-sm text-muted-foreground">
                <span>
                  Showing {((currentPage - 1) * itemsPerPage) + 1}-{Math.min(currentPage * itemsPerPage, filteredAndSortedNFTs.length)} of {filteredAndSortedNFTs.length} NFTs
                </span>
                ...
                <div className="flex flex-col items-center gap-3 pt-4">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-3 py-2 rounded-lg bg-secondary/20 hover:bg-secondary/40 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm"
                    >
                      First
                    </button>
                    ...
                    <button
                      ...
                      disabled={currentPage === 1}
                      className="px-4 py-2 rounded-lg bg-secondary/20 hover:bg-secondary/40 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                    >
                      Previous
                    </button>
                  ...
                  <div className="text-sm text-muted-foreground">
                    Page {currentPage} of {totalPages} ({filteredAndSortedNFTs.length} total NFTs)
                  </div>
                </div>
              )}
            </div>
